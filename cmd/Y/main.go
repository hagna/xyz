...
func getTLScerts(c, k, ca string) ([]byte, []byte, []byte, error) {
	...
	if err != nil {
		...
		cacert, cakey, err := xyz.MkCert(host, rsaBits, ecdsaCurve, validFrom, validFor, isCA, nil)
		if err != nil {
		...
		isCA = false
		cert, priv, err := xyz.CaSignedCert(host, rsaBits, ecdsaCurve, validFrom, validFor, isCA, &ca_key_pair)
		if err != nil {
